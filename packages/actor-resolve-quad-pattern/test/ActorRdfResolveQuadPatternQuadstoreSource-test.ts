...
import { ActorRdfResolveQuadPatternQuadstoreSource, QuadstoreQuadSource } from '..';
import 'jest-rdf';
...
describe('ActorRdfResolveQuadPatternRdfJsSource', () => {
  ...
  describe('The ActorRdfResolveQuadPatternQuadstoreSource module', () => {
    it('should be a function', () => {
      expect(ActorRdfResolveQuadPatternQuadstoreSource).toBeInstanceOf(Function);
    });

    it('should be a ActorRdfResolveQuadPatternQuadstoreSource constructor', () => {
      expect(new (<any> ActorRdfResolveQuadPatternQuadstoreSource)({ name: 'actor', bus }))
        .toBeInstanceOf(ActorRdfResolveQuadPatternQuadstoreSource);
      expect(new (<any> ActorRdfResolveQuadPatternQuadstoreSource)({ name: 'actor', bus }))
        .toBeInstanceOf(ActorRdfResolveQuadPattern);
    });

    it('should not be able to create new ActorRdfResolveQuadPatternQuadstoreSource objects without \'new\'', () => {
      expect(() => { (<any> ActorRdfResolveQuadPatternQuadstoreSource)(); }).toThrow();
    });
  });

  describe('An ActorRdfResolveQuadPatternQuadstoreSource instance', () => {
    let actor: ActorRdfResolveQuadPatternQuadstoreSource;
    let source: RDF.Source;

    beforeEach(() => {
      actor = new ActorRdfResolveQuadPatternQuadstoreSource({ name: 'actor', bus });
    ...
    it('should get the source', () => {
      return expect((<any> actor).getSource(ActionContext({ '@comunica/bus-rdf-resolve-quad-pattern:source':
          { type: 'rdfjsSource', value: source }})))
        .resolves.toMatchObject(new QuadstoreQuadSource(source));
    });
