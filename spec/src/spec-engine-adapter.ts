...
import { streamToArray } from './utils.js';
...
const indexes: ('subject'|'predicate'|'object'|'graph')[][] = [
...
async function source(data: Quad[]) {
  const { Quadstore } = await import('quadstore');
  const { Engine } = await import('quadstore-comunica');
...
const adapter: IQueryEngine & IUpdateEngine = {
  ...
  async parse(query, options) {
    const { __engine } = await import('quadstore-comunica');
    // @ts-ignore
  ...
  async query(data, queryString, options) {
    const { store, engine } = await source(data);
